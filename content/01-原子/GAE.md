...
## TD、MC、GAE 的关系：同一个东西的三个取值

**核心洞察**：TD、MC、GAE 不是三个不同的算法——它们是**同一个估计方法**在 $\lambda$ 取不同值时的三种表现形式。

| | 看多远 | 用什么估计未来 | $\lambda$ 值 |
|---|---|---|---|
| **TD** | 只看 1 步 | 全靠 $V$ 猜 | $\lambda = 0$ |
| **GAE** | 看很多步，逐渐过渡 | 近处靠实际奖励，远处靠 $V$ 猜 | $\lambda = 0.95$ |
| **MC** | 看完整局 | 完全靠实际奖励 | $\lambda = 1$ |

### 用同一个场景走一遍

你在玩一个游戏，当前在状态 $s_0$，跑了 4 步结束：

| 步 | $r_t$ | $V(s_t)$（Critic 估的） |
|----|-------|----------|
| $t=0$ | +1 | 5 |
| $t=1$ | +2 | 4 |
| $t=2$ | +3 | 3 |
...
$$A_0^{\text{TD}} = \underbrace{r_0 + V(s_1)}_{\text{这一步实际拿的 + 猜下一步}} - \underbrace{V(s_0)}_{\text{Critic 原本估的}} = 1 + 4 - 5 = \mathbf{0}$$

**问题**：$V(s_1) = 4$ 可能估错了！比如真实 $V(s_1)$ 应该是 15（因为后面有个 +10 的大奖励）。TD 完全不知道，因为它不看后面。

#### MC：跑完整局，回头看

$$A_0^{\text{MC}} = \underbrace{r_0 + r_1 + r_2 + r_3}_{\text{实际拿到的总分}} - \underbrace{V(s_0)}_{\text{Critic 原本估的}} = (1 + 2 + 3 + 10) - 5 = \mathbf{11}$$

**问题**：这局运气好拿了 +10，下局可能运气差只拿了 +1。每次结果差很远，方差大。

#### GAE：看很多步，逐步过渡到靠猜

$$A_0^{\text{GAE}} = \underbrace{1 \cdot \delta_0}_{\text{第 0 步的 TD}} + \underbrace{\lambda \cdot \delta_1}_{\text{第 1 步的 TD，打折}} + \underbrace{\lambda^2 \cdot \delta_2}_{\text{第 2 步的 TD，更打折}} + \cdots$$

先算每步 TD 误差 $\delta_t = r_t + V(s_{t+1}) - V(s_t)$：
...
$$A_0^{\text{GAE}} = 1 \cdot 0 + 0.5 \cdot 1 + 0.25 \cdot 0 = \mathbf{0.5}$$

#### 三种方法的对比

| | 公式 | 结果 | 问题 |
|---|---|---|---|
| TD（$\lambda=0$） | $\delta_0$ | **0** | 完全依赖 $V$，偏差大 |
| GAE（$\lambda=0.5$） | $\delta_0 + 0.5\delta_1 + 0.25\delta_2$ | **0.5** | 折中 |
| MC（$\lambda=1$） | $\delta_0 + \delta_1 + \delta_2$（消项后 = $G_0 - V(s_0)$） | **11** | 完全依赖实际奖励，方差大 |

**GAE 和 MC 的公式为什么一样？** 不是"一样"——当 $\lambda = 1$ 时，所有 $V$ 项互相消掉（telescoping），GAE **退化成了** MC。当 $\lambda = 0$ 时，只保留第一项，GAE **退化成了** TD。

### 可视化：看多远

```
TD (λ=0)        GAE (λ=0.95)              MC (λ=1)
┌──────┐        ┌──────────────────┐      ┌────────────────────────┐
│ 1步  │V猜     │ 1步 2步 3步 ... │V猜   │ 1步 2步 3步 ... 整局   │
│实际   │        │实际 实际 实际  猜│      │实际 实际 实际 ... 实际  │
└──────┘        └──────────────────┘      └────────────────────────┘
 ↑                                           ↑
只看1步                                  看完整局
其余全靠V猜                              完全不看V
...
GAE 的 $\lambda$ 控制"实际奖励"和"$V$ 猜测"之间的过渡：
- 近处的步（$\lambda^0 = 1$）：完全相信实际奖励
- 远处的步（$\lambda^{20} \approx 0.36$）：越来越依赖 $V$ 的猜测

**一句话总结**：TD、MC、GAE 是同一个估计方法在 $\lambda \in \{0, 1, 0.95\}$ 时的三种表现。$\lambda$ 越小越依赖 $V$（低方差高偏差），$\lambda$ 越大越依赖实际奖励（高方差低偏差）。
